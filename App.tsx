...
import { ProbeStatus, Point, Obstacle, FormationType } from './types';
import MapView from './components/MapView';
import GyroViz from './components/GyroViz';
import AudioVisualizer from './components/AudioVisualizer';

const BASE_LOCATION: Point = { x: 50, y: 50 };

type InteractionMode = 'NAV' | 'SCAN_AREA';
type CameraFeed = 'HD' | 'THERMAL';
...
const App: React.FC = () => {
  ...
  const [detectionAlert, setDetectionAlert] = useState<{ probeId: string, location: Point } | null>(null);
  const [interactionMode, setInteractionMode] = useState<InteractionMode>('NAV');
  const [activeCameraFeed, setActiveCameraFeed] = useState<CameraFeed>('HD');
  ...
  const simulationRef = useRef<number>(null);
  ...
  useEffect(() => {
    ...
    const initialObstacles: Obstacle[] = Array.from({ length: 15 }).map(() => ({
      x: 100 + Math.random() * 800,
      y: 100 + Math.random() * 600,
      radius: 15 + Math.random() * 25,
      type: 'rubble' as const
    }));
  ...
  const handleDetection = useCallback((probe: RubbleRatProbe) => {
  ...
  const applyFormation = (type: FormationType, center: Point) => {
    setProbes(prev => {
      const next = [...prev];
  ...
  const individualFocusedScan = (id: string) => {
    setProbes(prev => prev.map(p => {
      if (p.id === id) {
  ...
  const scanArea = (center: Point) => {
    let nearest: RubbleRatProbe | null = null;
    ...
    if (nearest) {
      setProbes(prev => prev.map(p => {
        if (p.id === (nearest as RubbleRatProbe).id) {
          p.targetX = center.x;
          p.targetY = center.y;
          p.status = ProbeStatus.SCANNING;
        }
  ...
  const handleAreaSelect = (rect: { x1: number, y1: number, x2: number, y2: number }) => {
...
const SensorCard: React.FC<{ 
  ...
  subLabel?: string,
  onAction?: () => void 
}> = ({ icon, label, value, alert, subLabel, onAction }) => (
  <div 
    onClick={onAction}
